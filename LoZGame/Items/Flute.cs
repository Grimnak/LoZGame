namespace LoZClone
{
    ...
    internal class Flute : ItemEssentials, IItem
    { 
        public Flute(Vector2 loc)
        {
            Sprite = ItemSpriteFactory.Instance.Flute();
            itemCollisionHandler = new ItemCollisionHandler(this);
            Physics = new Physics(loc);
            PickUpItemTime = LoZGame.Instance.UpdateSpeed;
            LifeTime = 0;
            Vector2 size = new Vector2(ItemSpriteFactory.FluteWidth, ItemSpriteFactory.FluteHeight);
