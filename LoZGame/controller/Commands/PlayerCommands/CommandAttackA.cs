namespace LoZClone
{
    ...
    public class CommandAttackA : ICommand
    {
        ...
        public void Execute()
        {
            if (!(this.player.State is DieState || this.player.State is PickupItemState || this.player.State is GrabbedState) && !LoZGame.Instance.Entities.ProjectileManager.PrimaryAttackLock)
            {
                this.player.Attack();
