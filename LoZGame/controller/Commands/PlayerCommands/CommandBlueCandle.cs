namespace LoZClone
{
    ...
    public class CommandBlueCandle : ICommand
    {
        ...
        public void Execute()
        {
            if (!LoZGame.Instance.Entities.ProjectileManager.FlameInUse && !(this.player.State is DieState || this.player.State is PickupItemState || this.player.State is GrabbedState))
            {
